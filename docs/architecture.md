...
Sodapop preferences live under the platform configuration directory. Runtime session state is account-scoped under Sodapop's state directory, with Windows using the local application-data directory rather than roaming configuration storage. Unix modes and Windows protected ACLs keep those files private, with project identity tracked for resume. Tokens belong only in a secure credential store or explicitly selected process memory.
